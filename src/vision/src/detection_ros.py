...
def ros_publisher():
    global center_segment_number
    pub = rospy.Publisher('center_segment', Int32, queue_size=10)
    
    rate = rospy.Rate(10)  # 10 Hz

    while not rospy.is_shutdown():
        if center_segment_number is not None:
            pub.publish(center_segment_number)
        rate.sleep()
...
if __name__ == "__main__":
    ...
    rospy.init_node('center_segment_publisher', anonymous=True)
